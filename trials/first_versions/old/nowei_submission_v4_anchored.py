"""STARE-LB v4 (REWARD + ANCHORED): v3 gate + device-relabel to cut transmit
...
Same reward gate as v3, plus an ANCHORED packing step: a device is just a
label, so after computing the balanced assignment we relabel device rows to
maximise expert-set overlap with the current layout. PAR is unchanged (loads per
device are identical) but transmit drops because more experts stay put.

Tunables: ALPHA, SAFETY, MIN_PAR.
"""
import numpy as np
import time
import torch
...
def rebalance(hotness, n_device, n_red_expert):
    ...
    # Defensive init when EP or n_phys changed between calls
    exp_per_dev = n_phys // n_device
    ...
    for L in cand_idx:
        ...
        # Defensive: ensure current stored deployment matches prop shape
