...
def rebalance(hotness, n_device, n_red_expert):
    ...
    # Defensive: ensure persistent state matches this run's shape; re-init if not
    exp_per_dev = n_phys // n_device
    expected_cur_shape = (n_layers, n_device, exp_per_dev)
    expected_fast_shape = (n_layers, n_experts)
    expected_slow_shape = (n_layers, n_experts)
    if _STATE.get("fast") is None or _STATE.get("fast").shape != expected_fast_shape or _STATE.get("slow") is None or _STATE.get("slow").shape != expected_slow_shape:
    ...
    w = np.minimum(w, cap)
    # Update EWMAs and ensure cur is available
    ...
    # Budget-aware candidate selection. EP32 keeps the full scan; EP64/EP128
    # use a hard top-K cap so the controller stays bounded and deterministic.
    if n_device <= 32:
        cand_idx = np.arange(n_layers)
    else:
        K = 4 if n_device <= 64 else 3
        K = min(K, n_layers)
