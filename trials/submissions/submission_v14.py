"""submission_v14.py -- v13 + DRIFT-ADAPTIVE PER-LAYER GATE.

THE COOKIE-RATIO CALL: PAR IS WORTH ~613,000 MOVES
==================================================
Per-case scoring math:
    total_time = 60.0 * mean_PAR + transmit * 9.787e-5
    => 1 PAR point per case == saving 60s == saving 613,000 moves.

User said: "even shelves > rest, always pick even shelves". The honest
answer is "yes, per case, up to 613k moves of PAR-buying spend". v13
uses GATE_SAFETY = 16 (require 16x more PAR justification than the
math break-even). On the leader's evidence (111.89 with 752k transit
vs our 524k) we are clearly under-spending on transit somewhere.

LOCAL SWEEP RESULTS (v13 base, full gate lowered globally over 7 LmSys cases):
    GATE=16  598.59s  baseline (v13 ship)
    GATE= 8  615.93s  (-2.9%)
    GATE= 4  705.88s  (-15%)
    GATE= 1  786.31s  (-24%)

Lowering the gate GLOBALLY catastrophically hurts LmSys because the
marginal moves it unlocks cost more transit than the PAR they buy.
Worked example, DS-R1/EP256, GATE=16->8:
    PAR drop 1.77 -> 1.44 = 0.33pt = 19.8s compute saved
    transit  18.9k -> 354k = +335k moves = 32.8s extra transit
    net: +13.0s LOSS

That's because LmSys is STATIONARY -- within-window drift = 0.024 --
so the predicted gain is well-calibrated and small. Spending transit
to chase 0.01 PAR drops on stationary data just doesn't pay.

BUT MIX IS NOT STATIONARY. Drift is large (estimated 0.3-0.7 from
trace structure); PAR sits at 3.67 on EP256. There the marginal
move's PAR payoff is much larger than 0.01, and lowering the gate
DOES pay back per the same cost math.

v14-A. DRIFT-ADAPTIVE PER-LAYER GATE
====================================
    drift_L  = ||hot_late_half_L - hot_early_half_L||_1
               / (||hot_early_L||_1 + ||hot_late_L||_1)             in [0, 1]
    safety_L = max(SAFETY_MIN, SAFETY_MAX - SAFETY_SLOPE * drift_L)
    threshold_L = TIME_PER_MOVE_S * n_layers * safety_L
                  / BALANCED_COMPUTE_SECONDS

With SAFETY_MAX = 16, SAFETY_MIN = 2, SAFETY_SLOPE = 30:
    drift_L = 0.02 (LmSys median)  -> safety_L = 15.4  (~v13)
    drift_L = 0.10                 -> safety_L = 13.0
    drift_L = 0.30                 -> safety_L =  7.0
    drift_L = 0.50                 -> safety_L =  2.0  (clipped to MIN)
    drift_L = 1.00                 -> safety_L =  2.0

LmSys stays at v13 behaviour. Mix layers fall into a regime where the
gate is 8x more permissive, unlocking the redeploys that v13 was
rejecting on drifty layers.

THE KEY DIFFERENCE VS v12 (which also tried drift-adaptive gate and
regressed to 110.91): v13 uses PREDICTED rate as the gate signal, not
past snapshot. So the gain estimate the looser gate compares against
is forward-looking and accurate, not backward-looking and noisy.

v14-B. KEEP v13's PREDICTIVE PLACEMENT + VARIANCE PACK UNCHANGED
=================================================================
    ALPHA_PREDICT = 1.0   (extrapolate full window forward)
    B_GAMMA       = 1.5   (pack against mean + 1.5*sigma)

Local-LmSys-safe by construction (drift=0.024 -> behavior == v13).
Mix-win expected by construction (drift>>0.024 -> gate loosens
exactly where prediction differs from past).
...
# GATE_SAFETY: per-move PAR threshold = TIME_PER_MOVE * n_layers * SAFETY
#                                       / BALANCED_COMPUTE_SECONDS
# Used only when DRIFT_ADAPTIVE_GATE_ENABLED = False (back-compat with v13).
GATE_SAFETY = 16.0

# v14-A: drift-adaptive per-layer gate. See module docstring.
# safety_L = max(SAFETY_MIN, SAFETY_MAX - SAFETY_SLOPE * drift_L)
#
# Local sweep on 7 LmSys cases (v13 baseline 598.590s):
#   MAX=16, SLOPE=30, MIN in {2,4,8} -> all 600.27s (-0.28%)
#   MAX=16, SLOPE=60, MIN in {2,4,8} -> all 597.38s (+0.20%) <-- shipped slope
#
# SLOPE=60 wins on LmSys because at drift=0.024 (LmSys median) safety_L =
# 16 - 60*0.024 = 14.56 -- the gate is 9% more permissive than v13's flat
# 16, picking up marginal moves whose PAR gain (computed on v13's
# predicted signal) is now reliable enough to act on.
#
# On Mix (estimated drift 0.3-0.7), MAX=16, SLOPE=60 takes safety_L into
# the MIN floor (drift > 0.233 clips at MIN=2). MIN=2 is the aggressive
# choice the user asked for: 2x the math break-even (which is 1.0), so
# we trust the predicted gain enough to spend transit but keep a small
# margin against catastrophic snapshot noise.
DRIFT_ADAPTIVE_GATE_ENABLED = True
GATE_SAFETY_MAX = 16.0   # == v13 (stationary-safe; LmSys layers stay near v13)
GATE_SAFETY_MIN = 2.0    # 2x math break-even; full aggression on Mix layers
GATE_SAFETY_SLOPE = 60.0 # +0.20% local; clip-to-MIN at drift_L > 0.233
...
def _per_layer_drift(hotness: np.ndarray) -> np.ndarray:
    """v14-A: per-layer first-half vs second-half L1 drift. Shape (L,).

    0 = stationary; ~1 = maximally non-stationary. LmSys sits at ~0.024,
    Mix expected ~0.3-0.7. Used to scale the per-layer gate safety.
    """
    W = hotness.shape[0]
    half = max(1, W // 2)
...
def rebalance(hotness, n_device, n_red_expert):
    ...
    # ---------- v14-A: per-layer drift-adaptive gate threshold --------------
    # Pre-compute per-layer safety once per call so the inner loop only
    # indexes. Stationary layers (low drift) keep v13's GATE_SAFETY_MAX;
    # drifty Mix layers fall to GATE_SAFETY_MIN.
    gate_base = _TIME_PER_MOVE_S * n_layers / _BALANCED_COMPUTE_SECONDS
    if DRIFT_ADAPTIVE_GATE_ENABLED:
        drift = _per_layer_drift(hotness)                         # (L,)
        safety_per_layer = np.maximum(
            GATE_SAFETY_MIN,
            GATE_SAFETY_MAX - GATE_SAFETY_SLOPE * drift,
        )
        threshold_per_layer = gate_base * safety_per_layer
    else:
        threshold_per_layer = np.full(n_layers,
                                       gate_base * GATE_SAFETY,
                                       dtype=np.float64)
