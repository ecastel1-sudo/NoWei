...
### Not literal TCP — same idea

We **borrow the control philosophy** from TCP congestion control
(Jacobson, SIGCOMM '88): react to what actually happened, grow trust
on confirmation, shrink it on failure, keep a bounded dial. We do
**not** implement TCP-Reno (no packets, no RTT, no additive `cwnd +=
1`). v15 is a **MIMD-style** per-layer gate on PAR prediction
residuals — inverted semantics (smaller `safety` = braver), plus a
dead zone. "TCP-AIMD" here means *inspired by*, not *is*.
...
## 7. Key takeaways (explain-it-like-I'm-5) ;)

Each layer gets its own **trust dial**. We promise a result, check it
next round, and turn the dial.

- **Promise.** On every commit we write down: *"this layer's PAR will
be X."*
- **Check.** Next call we measure: *was it X?* → `err = actual / predicted`.
- **Kept promise → braver.** `err ≤ 1.05` → dial × **0.85**. Move
more freely next time.
- **Broken promise → shyer.** `err ≥ 1.30` → dial × **1.30**. Demand
more proof next time.
- **Meh promise → ignore.** Inside `[1.05, 1.30]` we don't touch it.
Noise shouldn't move the dial.
