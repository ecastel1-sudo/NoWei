"""STARE-LB v1 (BASE): EWMA + spike-clip + PAR-gain gating
...
Feeds DeepSeek's replicate+pack core a TIME-SMOOTHED load (EWMA over windows
with per-window p99 clipping) so transient spikes are ignored. Redeploys a layer
only when predicted PAR improvement clears a fixed gain floor; best gain-per-move
first. Stable/cold layers keep their placement -> low transmit.

Tunables: ALPHA (smaller=longer memory), PAR_GAIN_THRESHOLD, CLIP_PCTL.
"""
import numpy as np
import torch
...
def rebalance(hotness, n_device, n_red_expert):
    ...
    for L in range(n_layers):
        ...
        if cur_par >= MIN_PAR and gain >= PAR_GAIN_THRESHOLD and cost > 0:
